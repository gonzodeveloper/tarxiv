...
class TarxivTNS:
    ...
    def __init__(self, config_dir, log_name=None):
        ...
        log_file = os.path.join(self.config['log_dir'], log_name) if log_name is not None else None
        ...
        if os.path.exists(token):
            self.creds = Credentials.from_authorized_user_file(token, self.config['gmail']['scopes'])
        # If there are no (valid) credentials available, let the user log in.
        if not self.creds or not self.creds.valid:
            flow = InstalledAppFlow.from_client_secrets_file(secrets, self.config['gmail']['scopes'])
    ...
    def download_bulk_tns(self):
        ...
        get_url = self.site + "/system/files/tns_public_objects/tns_public_objects.csv.zip"
        json_data = [("api_key", (None, self.api_key)),]
        headers = {'User-Agent': self.marker}
